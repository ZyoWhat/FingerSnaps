...
export default function WorkList({ projects, onBack }: WorkListProps) {
  ...
  const Section = ({ title, items }: { title: string, items: Project[] }) => (
    <div className="mb-20 w-full max-w-3xl">
      <h2 className="text-[10px] md:text-xs font-bold tracking-[0.3em] text-black/30 mb-10 md:mb-12 uppercase px-6 md:px-0">
        [{title}]
      </h2>
      <div className="flex flex-col gap-10 md:gap-6">
        {items.map((item, idx) => (
          <motion.div
            ...
            className="group flex items-center justify-between border-b border-black/5 pb-4 hover:border-black/20 transition-colors px-6 md:px-0"
          >
            <span className="text-lg md:text-xl font-medium tracking-tight">
              {item.name}
            </span>
            
            {/* Image Container: Hidden until hover/tap for identical behavior */}
            <div className="w-20 md:w-32 h-12 md:h-20 overflow-hidden bg-gray-50 rounded-sm opacity-0 group-hover:opacity-100 transition-all duration-500 shadow-sm md:shadow-none pointer-events-none">
              <img 
